...
export default function DisputeBubbleField({ chargebacks = [], filtered = [] }) {
  ...
  return (
    ...
    >
      ...
      {/* TOP HEADER */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, position: 'relative', zIndex: 5 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '430px 1fr', gap: 32, alignItems: 'center', position: 'relative', zIndex: 4, marginBottom: 20 }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.015)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 18, padding: '18px 20px' }}>
          ...
          {/* Bigger Dial SVG Viewport */}
          <div style={{ position: 'relative', width: 390, height: 320, margin: '0 auto' }}>
            <svg width="390" height="320" viewBox="0 0 420 320" style={{ overflow: 'visible' }}>
              <defs>
              ...
              {/* Leader Line to OPEN (Left) */}
              <path
                d="M 96 160 L 70 160"
                fill="none"
                stroke="rgba(255,82,113,0.5)"
                strokeWidth="1.5"
              />

              {/* Leader Line to CLOSED (Top Right) */}
              <path
                d="M 248 54 L 290 32"
                fill="none"
              ...
              {/* Leader Line to REJECTED (Bottom Right) */}
              <path
                d="M 248 266 L 290 286"
                fill="none"
              ...
              {slices.map((slice) => {
                ...
                return (
                  <g
                    key={slice.key}
                    onMouseEnter={() => setHoveredStatus(slice.key)}
              ...
              <text x={cx} y={cy + 6} textAnchor="middle" fill="#ffffff" fontSize="24" fontWeight="800" fontFamily="var(--font-mono)">
                2,800
              </text>
              <text x={cx} y={cy + 25} textAnchor="middle" fill="var(--text-2)" fontSize="11" fontWeight="600">
                Total Disputes
              </text>
              <text x={cx} y={cy + 42} textAnchor="middle" fill="var(--lime)" fontSize="10.5" fontWeight="700">
                ↑ 12% <tspan fill="var(--text-2)" fontWeight="400">vs. last month</tspan>
              </text>
            </svg>

            {/* CALLOUT CARD 1: OPEN (Left) */}
            <div
              onMouseEnter={() => setHoveredStatus('OPEN')}
              onMouseLeave={() => setHoveredStatus(null)}
              style={{
                position: 'absolute',
                left: 0,
                top: 110,
                background: 'rgba(255, 82, 113, 0.08)',
                border: `1px solid ${hoveredStatus === 'OPEN' ? '#ff5271' : 'rgba(255, 82, 113, 0.3)'}`,
                borderRadius: 10,
                padding: '8px 12px',
                minWidth: 78,
                boxShadow: hoveredStatus === 'OPEN' ? '0 0 16px rgba(255,82,113,0.3)' : 'none',
                transition: 'all 0.15s ease',
                cursor: 'pointer',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 2 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#ff5271' }} />
                <span style={{ fontSize: 10, fontWeight: 800, color: '#ff5271', letterSpacing: 0.5 }}>OPEN</span>
              </div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 16, fontWeight: 800, color: '#ffffff' }}>
                {metricMode === 'count' ? '1,492' : '₹42.5L'}
              </div>
            ...
            {/* CALLOUT CARD 2: CLOSED (Top Right) */}
            <div
              onMouseEnter={() => setHoveredStatus('CLOSED')}
              onMouseLeave={() => setHoveredStatus(null)}
              style={{
                position: 'absolute',
                right: 0,
                ...
                border: `1px solid ${hoveredStatus === 'CLOSED' ? '#3ddc97' : 'rgba(61, 220, 151, 0.3)'}`,
                borderRadius: 10,
                padding: '8px 12px',
                minWidth: 78,
                boxShadow: hoveredStatus === 'CLOSED' ? '0 0 16px rgba(61,220,151,0.3)' : 'none',
                transition: 'all 0.15s ease',
                cursor: 'pointer',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 2 }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 800, color: '#3ddc97', letterSpacing: 0.5 }}>CLOSED</span>
              </div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 16, fontWeight: 800, color: '#ffffff' }}>
                {metricMode === 'count' ? '865' : '₹21.0L'}
              </div>
              <div style={{ fontSize: 11, color: 'var(--text-2)', fontFamily: 'var(--font-mono)' }}>
                30.9%
              </div>
            ...
            {/* CALLOUT CARD 3: REJECTED (Bottom Right) */}
            <div
              ...
              style={{
                ...
                border: `1px solid ${hoveredStatus === 'REJECTED' ? '#a78bfa' : 'rgba(167, 139, 250, 0.3)'}`,
                borderRadius: 10,
                padding: '8px 12px',
                minWidth: 78,
                boxShadow: hoveredStatus === 'REJECTED' ? '0 0 16px rgba(167,139,250,0.3)' : 'none',
                transition: 'all 0.15s ease',
                cursor: 'pointer',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 2 }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 800, color: '#a78bfa', letterSpacing: 0.5 }}>REJECTED</span>
              </div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 16, fontWeight: 800, color: '#ffffff' }}>
                {metricMode === 'count' ? '443' : '₹9.8L'}
              </div>
              <div style={{ fontSize: 11, color: 'var(--text-2)', fontFamily: 'var(--font-mono)' }}>
                15.8%
              </div>
        ...
        {/* RIGHT COLUMN: How Disputes Reach the Bank (Seamless Volume Stream Tracks with Zero Empty Gap) */}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--text-2)', marginTop: 2 }}>
                Channels through which customers raise disputes.
              </div>
            </div>
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-2)', background: 'rgba(255,255,255,0.04)', padding: '3px 8px', borderRadius: 9999 }}>
              5 Ingestion Vectors
            </span>
          </div>

          {/* S-Flow Constellation Container with Seamlessly Flowing Progress Tracks */}
          <div style={{ position: 'relative', height: 280 }}>
            
            {/* SVG S-Curve River connecting the 5 bubble centers */}
            <svg
              style={{
                position: 'absolute',
                top: 0,
            ...
            >
              ...
              {/* Glowing winding S-spline */}
              <path
                d="M 28 26 C 28 52, 52 52, 52 80 C 52 106, 72 106, 72 134 C 72 160, 88 160, 88 188 C 88 214, 102 214, 102 242"
                fill="none"
            ...
            {/* 5 Channel Rows with full flex-1 bridging progress tracks (No Gaps!) */}
            {CHANNELS.map((ch, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ paddingLeft: ch.x, display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                    <div
                      style={{
                        ...
                        borderRadius: '50%',
                        background: '#131824',
                        border: `2px solid ${isHovered ? '#ffffff' : ch.color}`,
                        boxShadow: `0 0 16px ${ch.glow}, inset 0 1px 2px rgba(255,255,255,0.4)`,
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        fontSize: 14,
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, minWidth: 110 }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13.5, fontWeight: 800, color: ch.color }}>
                  ...
                  {/* Dynamic Progress Stream Bar (takes all middle space seamlessly!) */}
                  <div style={{ flex: 1, minWidth: 80, height: 7, background: 'rgba(255,255,255,0.06)', borderRadius: 4, overflow: 'hidden', position: 'relative' }}>
                    <div 
                      style={{ 
                        width: `${barWidthPct}%`, 
                        height: '100%', 
                        borderRadius: 4, 
                        background: `linear-gradient(90deg, ${ch.color}55 0%, ${ch.color} 100%)`,
                        boxShadow: isHovered ? `0 0 12px ${ch.color}` : 'none',
                        transition: 'width 0.4s ease, box-shadow 0.2s',
                      }} 
                    />
                  </div>

                  {/* Right: Description Tag seamlessly docked */}
                  ...
                  {isHovered && (
                    <div
                      style={{
                        ...
                        borderRadius: 10,
                        padding: '7px 13px',
                        boxShadow: '0 8px 24px rgba(0,0,0,0.7), 0 0 16px rgba(61,220,151,0.25)',
                    ...
                    >
                      ...
                      <div style={{ fontSize: 10.5, color: ch.color, fontFamily: 'var(--font-mono)', marginTop: 1 }}>
                        {metricMode === 'count' ? `${ch.count} disputes (${ch.pct})` : `${fmtINR(ch.count * 2800)} (${ch.pct})`}
                      </div>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: 'var(--lime)', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>
